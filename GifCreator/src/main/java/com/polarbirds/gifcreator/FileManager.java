...
public class FileManager {
    ...
    public void saveGif(byte[] imageByteArray, File file) {
        ...
        try (FileOutputStream fos = new FileOutputStream(file); ImageOutputStream imageOutputStream = ImageIO.createImageOutputStream(fos)) {
            imageOutputStream.write(imageByteArray);
            System.out.println("Done writing gif. Stream pos: " + imageOutputStream.getStreamPosition());
        } catch (IOException e) {
            System.err.println("Writing gif failed: " + file.getAbsolutePath());
            e.printStackTrace();
        }
    }
